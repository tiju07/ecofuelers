...
const AlertCard: React.FC = () => {
  ...
  return (
    <div className='container mx-auto p-4'>
      <h1 className='mb-4 text-2xl font-bold text-[#2E7D32]'>Waste Reduction Alerts</h1>
      {loading ? (
        ...
        <div className='grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-3'>
          {alerts.map((alert) => (
            <div
              key={alert.id}
              className={`${ alert.urgency === "high" ? "border-red-500" : alert.urgency === "medium" ? "border-yellow-500" : "border-green-500" } rounded border p-4 shadow-md`}
            >
              <h2 className='text-lg font-bold text-gray-800'>{alert.supply_name}</h2>
              <p className='text-gray-700'>Issue: {alert.issue}</p>
              <p
                className={`${ alert.urgency === "high" ? "text-red-500" : alert.urgency === "medium" ? "text-yellow-500" : "text-green-500" } font-semibold`}
              >
                Urgency: {alert.urgency.charAt(0).toUpperCase() + alert.urgency.slice(1)}
              </p>
              <button
                onClick={() => dismissAlert(alert.id)}
                className='mt-4 rounded bg-[#2E7D32] px-4 py-2 text-white transition duration-300 hover:bg-[#1B5E20]'
              >
                Dismiss
              </button>
            </div>
